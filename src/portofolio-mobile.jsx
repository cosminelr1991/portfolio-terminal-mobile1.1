...
function calcPortfolio(liveQuotes) {
  return PORTFOLIO_DATA.map(s => {
    ...
    // DacÄƒ avem divRate live, Ã®l folosim pentru annualDiv; altfel recalculÄƒm din yield
...
function ThemeToggle() {
  ...
  return (
    <button
      onClick={toggleTheme}
      title={isDark ? "Mod luminos" : "Mod Ã®ntunecat"}
      style={{
...
function TopBar({ onRefresh, liveStatus, lastUpdate }) {
  ...
  const statusDot = {
    idle:    { color: THEME.dim,   label: "â€”" },
    loading: { color: THEME.gold,  label: "âŸ³ sync..." },
    live:    { color: THEME.green, label: `live Â· ${lastUpdate ? lastUpdate.toLocaleTimeString("ro-RO", { hour: "2-digit", minute: "2-digit", second: "2-digit" }) : ""}` },
    error:   { color: THEME.red,   label: "offline Â· date demo" },
  }[liveStatus] ?? { color: THEME.dim, label: "â€”" };
...
function SummaryMetrics({ totals }) {
  ...
  const metrics = [
    { label: "VALOARE TOTALÄ‚", value: `$${fmt(totals.value)}`, delta: null },
    ...
    { label: "EVOLUÈšIE ZILNICÄ‚", value: `${sign(totals.dailyChgUSD)}$${fmt(Math.abs(totals.dailyChgUSD))}`, delta: `${sign(totals.dailyChgPct)}${fmt(totals.dailyChgPct, 2)}%`, dc: clr(totals.dailyChgUSD) },
        ...
        <div style={{ margin: "0 12px", background: THEME.surface, border: `1px solid ${THEME.border}`, borderRadius: 12, overflow: "hidden", boxShadow: "0 -4px 24px rgba(0,0,0,0.5)" }}>
          {moreItems.map((item, i) => (
            <button key={item.id} onClick={() => handleMore(item.id)}
              ...
              {active === item.id && <span style={{ color: THEME.gold, fontSize: 12 }}>â—</span>}
            </button>
          ))}
        </div>
      </div>
...
// â”€â”€ DONUT CHART COMPONENT â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ TAB 1: MATRICEA PORTOFOLIULUI â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function MatriceTab({ portfolio, totals }) {
  ...
  // Donut 1 â€” Harta ProfitabilitÄƒÈ›ii (per sector, colorat by profit avg)
  const sectors = {};
  portfolio.forEach(s => {
  ...
  // Donut 2 â€” EvoluÈ›ie vs PreÈ› Mediu (per stock, color by profitPct)
  ...
  return (
    <div style={{ padding: "16px 12px", display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", borderBottom: `1px solid ${THEME.border}` }}>
        {[{ id: "harti", label: "HÄƒrÈ›i Portofoliu" }, { id: "miscari", label: "MiÈ™cÄƒrile Zilei" }, { id: "registru", label: "Registru" }].map(t => (
      ...
      {matSubTab === "harti" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          {/* Donut 1: Harta ProfitabilitÄƒÈ›ii */}
          <section>
            <SectionHeader>HARTA PROFITABILITÄ‚ÈšII</SectionHeader>
            <Card>
              <DonutChart
                slices={donut1Slices}
                  ...
                  {(() => {
                    ...
                    return (
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 }}>
                        ...
                        <div><div style={{ fontSize: 8, color: THEME.dim }}>AcÈ›iuni</div><div style={{ fontSize: 10, color: THEME.text }}>{secData.stocks.map(s => s.symbol).join(", ")}</div></div>
                      </div>
                    );
                  })()}
                </div>
          ...
          {/* Donut 2: EvoluÈ›ie vs PreÈ› Mediu */}
          <section>
            <SectionHeader>EVOLUÈšIE VS PREÈš MEDIU</SectionHeader>
            <Card>
              <DonutChart
                slices={donut2Slices}
              ...
              <div style={{ display: "flex", justifyContent: "center", gap: 10, marginTop: 4, flexWrap: "wrap" }}>
                {[
                  ...
                  { color: "#2ECC71", label: "+20â€“50%" },
                  { color: "#58d68d", label: "+5â€“20%" },
                  { color: "#a9dfbf", label: "0â€“5%" },
              ...
              {hoveredDonut2 !== null && (() => {
                ...
                return (
                  <div style={{ marginTop: 10, background: THEME.bg, borderRadius: 6, padding: "10px 12px", borderLeft: `3px solid ${donut2Slices[hoveredDonut2].color}` }}>
                    <div style={{ fontSize: 11, color: THEME.text, marginBottom: 6 }}><span style={{ fontFamily: FONT_MONO, color: THEME.gold }}>{sym}</span> â€” {st.name}</div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6 }}>
                      <div><div style={{ fontSize: 8, color: THEME.dim }}>PreÈ› curent</div><div style={{ fontFamily: FONT_MONO, fontSize: 11 }}>${fmt(st.price)}</div></div>
      ...
      {/* â”€â”€ MIÈ˜CÄ‚RILE ZILEI â”€â”€ */}
      {matSubTab === "miscari" && (
        <section>
          <SectionHeader>MIÈ˜CÄ‚RILE ZILEI â€” TOP 5</SectionHeader>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div>
              <div style={{ fontSize: 9, color: THEME.green, letterSpacing: 1, textTransform: "uppercase", marginBottom: 6 }}>â–² CreÈ™teri</div>
              {gainers.map(s => (
            ...
            <div>
              <div style={{ fontSize: 9, color: THEME.red, letterSpacing: 1, textTransform: "uppercase", marginBottom: 6 }}>â–¼ ScÄƒderi</div>
              {losers.map(s => (
                <div key={s.symbol} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: `1px solid ${THEME.border}` }}>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 500, color: THEME.text }}>{s.symbol}</div>
...
// â”€â”€ TAB 2: DIAGNOZÄ‚ AI â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function DiagTab({ portfolio, totals }) {
  ...
  const runAI = useCallback(async () => {
    ...
    const prompt = `EÈ™ti un analist financiar senior. AnalizeazÄƒ acest portofoliu È™i oferÄƒ comentariu profesionist Ã®n romÃ¢nÄƒ (max 250 cuvinte, fÄƒrÄƒ markdown, text simplu cu secÈ›iuni pe linii noi).\n\nPortofoliu: Valoare $${totals.value.toFixed(0)}, Profit $${totals.profit.toFixed(0)} (${pct}%), Dividende anuale $${totals.divIncome.toFixed(0)}\n\n${lines}\n\nStructurÄƒ rÄƒspuns:\nREZUMAT EXECUTIV (2-3 fraze)\nPUNCTE FORTE (max 3, cu ticker-ele specifice)\nRISCURI IDENTIFICATE (max 3 concrete)\nRECOMANDÄ‚RI (max 3 acÈ›iuni concrete)`;
    try {
  ...
  const flags = [
    { label: "Evaluare ExcesivÄƒ (P/E > 35)", color: THEME.gold, items: portfolio.filter(s => s.pe > 35 && s.pe < 500).sort((a, b) => b.pe - a.pe), val: s => `P/E ${s.pe.toFixed(1)}` },
    { label: "Risc TÄƒiere Dividend (Payout > 80%)", color: THEME.red, items: portfolio.filter(s => s.payout > 80 && s.annualDiv > 0 && s.sector !== "Real Estate").sort((a, b) => b.payout - a.payout), val: s => `${s.payout.toFixed(0)}%` },
    { label: "Volatilitate RidicatÄƒ (Beta > 1.5)", color: THEME.blue, items: portfolio.filter(s => s.beta > 1.5).sort((a, b) => b.beta - a.beta), val: s => `Î² ${s.beta.toFixed(2)}` },
    { label: "Lichiditate ScÄƒzutÄƒ (CR < 1)", color: THEME.red, items: portfolio.filter(s => s.currentRatio > 0 && s.currentRatio < 1 && s.sector !== "Financials").sort((a, b) => a.currentRatio - b.currentRatio), val: s => `CR ${s.currentRatio.toFixed(2)}` },
    { label: "MarjÄƒ Profit NegativÄƒ", color: THEME.red, items: portfolio.filter(s => s.profitMargin < 0), val: s => `${(s.profitMargin * 100).toFixed(1)}%` },
    { label: "Concentrare ExcesivÄƒ (> 15%)", color: THEME.gold, items: portfolio.filter(s => (s.value / totals.value) * 100 > 15).sort((a, b) => b.value - a.value), val: s => `${((s.value / totals.value) * 100).toFixed(1)}%` },
  ];

  return (
    <div style={{ padding: "16px 12px", display: "flex", flexDirection: "column", gap: 16 }}>
      <Card accent={THEME.gold}>
        <SectionHeader>DIAGNOZÄ‚ NARATIVÄ‚ â€” CLAUDE AI</SectionHeader>
        <button onClick={runAI} disabled={loading} style={{ background: "transparent", border: `1px solid ${THEME.gold}`, color: THEME.gold, borderRadius: 6, padding: "8px 14px", fontSize: 12, cursor: "pointer", fontFamily: "inherit", marginBottom: 12, opacity: loading ? 0.6 : 1, width: "100%" }}>
          {loading ? "â³ Se analizeazÄƒ portofoliul..." : "ðŸ§  GenereazÄƒ AnalizÄƒ AI a Portofoliului"}
        </button>
        {aiText ? (
          ...
          <div style={{ fontSize: 11, color: THEME.dim, fontStyle: "italic" }}>ApasÄƒ butonul pentru analizÄƒ narativÄƒ AI a portofoliului.</div>
        )}
      </Card>

      <SectionHeader>RAPORT DE SÄ‚NÄ‚TATE â€” ALERTE AUTOMATE</SectionHeader>
      {flags.map(f => (
        <Card key={f.label} accent={f.color}>
          <div style={{ fontSize: 10, color: f.color, letterSpacing: 0.5, marginBottom: 8 }}>{f.label}</div>
          {f.items.length === 0 ? (
            <div style={{ fontSize: 11, color: THEME.dim }}>âœ“ Nicio problemÄƒ detectatÄƒ</div>
          ) : f.items.map(s => (
...
// â”€â”€ TAB 3: FLUXURI & EVENIMENTE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    <div style={{ padding: "16px 12px", display: "flex", flexDirection: "column", gap: 20 }}>
      <Card accent={THEME.gold}>
        ...
        <div style={{ display: "flex", gap: 16, marginTop: 4 }}>
          <div style={{ fontSize: 10, color: THEME.dim }}>${fmt(totalAnnual / 12)}<span style={{ color: THEME.dim }}> / lunÄƒ medie</span></div>
          <div style={{ fontSize: 10, color: THEME.dim }}>${fmt(totalAnnual / 52)}<span style={{ color: THEME.dim }}> / sÄƒptÄƒmÃ¢nÄƒ</span></div>
        </div>
      </Card>

      <section>
        <SectionHeader>PROIECÈšIE FLUX LUNAR DIVIDENDE</SectionHeader>
            ...
            return (
              <div key={luna}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <div style={{ fontFamily: FONT_MONO, fontSize: 11, color: val > 0 ? THEME.gold : THEME.dim, width: 55, textAlign: "right", flexShrink: 0 }}>{val > 0 ? `$${fmt(val, 0)}` : "â€”"}</div>
                </div>
              </div>
            );
          })}
      ...
      <section>
        <SectionHeader>DISTRIBUÈšIE NOMINALÄ‚ DIVIDENDE</SectionHeader>
        {divStocks.map(s => (
          <div key={s.symbol} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: `1px solid ${THEME.border}` }}>
            <div>
              ...
              <div style={{ fontSize: 9, color: THEME.dim }}>{s.shares} acÈ› Â· {fmt(s.divYield, 2)}% yield Â· {(DIV_MONTHS[s.symbol] || []).length}x/an</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontFamily: FONT_MONO, fontSize: 13, color: THEME.gold }}>${fmt(s.annualDiv)}/an</div>
              <div style={{ fontSize: 10, color: THEME.dim }}>${fmt(s.annualDiv / 12, 2)}/lunÄƒ</div>
            </div>
          </div>
        ))}
      </section>
      ...
      <section>
        ...
        <Card>
          <div style={{ fontSize: 11, color: THEME.dim, marginBottom: 10 }}>Date aproximative â€” verificaÈ›i brokerul pentru confirmare</div>
          {portfolio.map(s => (
...
// â”€â”€ TAB 4: REBALANSARE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function RebalTab({ portfolio, totals }) {
  ...
  return (
    <div style={{ padding: "16px 12px", display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <section>
        <SectionHeader>PONDERI CURENTE VS ÈšINTÄ‚</SectionHeader>
        {portfolio.map(s => {
            ...
            <div key={s.symbol} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderBottom: `1px solid ${THEME.border}` }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                  ...
                  <span style={{ fontSize: 9, color: THEME.gold }}>ÈšintÄƒ: {fmt(targets[s.symbol], 1)}%</span>
                </div>
      ...
      {Math.abs(sumaTargets - 100) <= 0.1 && suggestions.length > 0 && (
        <section>
          <SectionHeader>VECTORI DE ACHIZIÈšIE</SectionHeader>
          {suggestions.map(s => (
            <div key={s.symbol} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: `1px solid ${THEME.border}` }}>
              <div>
                <div style={{ fontFamily: FONT_MONO, fontSize: 13, color: THEME.gold }}>{s.symbol}</div>
                <div style={{ fontSize: 10, color: THEME.dim }}>${fmt(s.price)} / acÈ›iune</div>
              </div>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontFamily: FONT_MONO, fontSize: 13, color: THEME.green }}>+{s.shares} acÈ›.</div>
...
// â”€â”€ TAB 5: WATCHLIST â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function WatchlistTab({ portfolio, totals }) {
  ...
  const scored = portfolio.map(s => {
    ...
    if (s.divYield > 0) {
      ...
      else if (s.payout <= 65) { score += 14; reasons.push("payout sÄƒnÄƒtos"); }
      else if (s.payout <= 85) { score += 7; }
    }
    if (s.dailyChg < 0) { score += Math.min(10, Math.abs(s.dailyChg) * 2); reasons.push("slÄƒbiciune zilnicÄƒ"); }
    if (weight <= medianWeight) { score += 6; reasons.push("pondere sub medianÄƒ"); }
  ...
  const getSignal = (score) => {
    ...
    if (score >= 50) return { label: "De UrmÄƒrit", color: THEME.gold, bg: "rgba(232,196,104,0.12)" };
    return { label: "Candidat", color: THEME.blue, bg: "rgba(74,158,255,0.12)" };
  };

  return (
    <div style={{ padding: "16px 12px", display: "flex", flexDirection: "column", gap: 16 }}>
      <SectionHeader>WATCHLIST â€” OPORTUNITÄ‚ÈšI DCA</SectionHeader>
        ...
        return (
          <Card key={s.symbol}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5, marginBottom: 8 }}>
              {[
                { l: "PreÈ› Live", v: `$${fmt(s.price)}` },
                { l: "Cost Mediu", v: `$${fmt(s.avgCost)}` },
                { l: "P/E", v: s.pe > 0 ? s.pe.toFixed(1) : "N/A" },
                { l: "Div Yield", v: `${s.divYield.toFixed(2)}%` },
                { l: "Discount", v: s.discount > 0 ? `${s.discount.toFixed(1)}%` : "â€”", c: s.discount > 0 ? THEME.green : THEME.dim },
            ...
            {s.reasons.length > 0 && <div style={{ fontSize: 10, color: THEME.dim }}>{s.reasons.join(" Â· ")}</div>}
          </Card>
        );
      })}
    </div>
...
// â”€â”€ DEEP DIVE CHART COMPONENTS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

// 1. ISTORICUL PREÈšULUI 365 ZILE cu MA50 + cost mediu
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 14, marginTop: 4, fontSize: 8, color: THEME.dim, flexWrap: "wrap" }}>
        <span><span style={{ color: THEME.blue }}>â€”â€”</span> Close</span>
        ...
        <span style={{ marginLeft: "auto", fontFamily: FONT_MONO, color: THEME.text }}>
          52W: ${Math.min(...closes).toFixed(2)} â€“ ${Math.max(...closes).toFixed(2)}
        </span>
      </div>
    </div>
  );
...
function FCFDividendChart({ stock }) {
  // GenerÄƒm 4 ani de date istorice estimate din fundamentalele disponibile
  ...
  // FCF/Share estimat: div / payout * 100, cu variaÈ›ie
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 14, marginTop: 4, fontSize: 8, color: THEME.dim, alignItems: "center" }}>
        <span><span style={{ color: THEME.green }}>â–® </span>FCF / Share</span>
        <span><span style={{ color: THEME.gold }}>â–® </span>Div / Share</span>
...
// 3. EVOLUÈšIE VENITURI (estimat din mktCap + profitMargin)
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 14, marginTop: 4, fontSize: 8, color: THEME.dim }}>
        <span><span style={{ color: THEME.blue }}>â–® </span>CreÈ™tere</span>
        <span><span style={{ color: THEME.red }}>â–® </span>ScÄƒdere</span>
        <span><span style={{ color: THEME.gold }}>â€”â€”</span> Trend</span>
...
// 4. MONITORIZARE BUYBACK (evoluÈ›ie acÈ›iuni Ã®n circulaÈ›ie estimatÄƒ)
function BuybackChart({ stock }) {
  ...
  const years = [currentYear - 3, currentYear - 2, currentYear - 1, currentYear];
  // Simulate buyback history â€” dividend payers tend to reduce shares
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: H }}>
        ...
        {/* Bars cu culoare: verde dacÄƒ scade (buyback), roÈ™u dacÄƒ creÈ™te (dilution) */}
        {data.map((d, i) => {
      ...
      {(() => {
        ...
        return (
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6, fontSize: 8, color: THEME.dim }}>
            <span>
              <span style={{ color: THEME.green }}>â–® </span>Buyback (scÄƒdere){"  "}
              <span style={{ color: THEME.red }}>â–® </span>DiluÈ›ie (creÈ™tere)
            </span>
            <span style={{ fontFamily: FONT_MONO, color: isBuyback ? THEME.green : THEME.red }}>
              {isBuyback ? "â–¼" : "â–²"} {Math.abs(chg).toFixed(1)}% {isBuyback ? "buyback net" : "diluÈ›ie net"}
            </span>
          </div>
        );
      })()}
      <div style={{ fontSize: 9, color: THEME.dim, fontStyle: "italic", marginTop: 2 }}>
        * Date estimate pe baza capitalizÄƒrii de piaÈ›Äƒ
      </div>
    </div>
  );
}

// â”€â”€ TAB 6: DEEP DIVE (FULL) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginTop: 8 }}>
        {[
          ...
          { l: "Volatilitate AnualÄƒ", v: `${annVol.toFixed(1)}%`, good: annVol < 25 },
          { l: "Cel mai rÄƒu Drawdown", v: `${maxDD.toFixed(2)}%`, good: maxDD > -5 },
          { l: "Medie ZilnicÄƒ", v: `${mean.toFixed(3)}%`, good: mean > 0 },
        ].map(x => (
...
function DeepDiveTab({ portfolio, totals }) {
  ...
  const scorecard = [
    { label: "Net Profit Margin", value: `${(s.profitMargin * 100).toFixed(1)}%`, good: s.profitMargin > 0.10, note: "Profitabilitate netÄƒ" },
    { label: "Return on Equity (ROE)", value: `${(s.roe * 100).toFixed(1)}%`, good: s.roe > 0.10, note: "EficienÈ›a capitalului propriu" },
    { label: "Debt / Equity", value: `${s.debtEq.toFixed(2)}x`, good: s.debtEq < 1.0, note: "Levierul financiar" },
    { label: "Current Ratio", value: `${s.currentRatio.toFixed(2)}x`, good: s.currentRatio >= 1.5, note: "Capacitate platÄƒ termen scurt" },
    { label: "Payout Ratio", value: `${s.payout.toFixed(1)}%`, good: s.payout <= 65, note: "Sustenabilitatea dividendului" },
    { label: "P/E Ratio", value: s.pe > 0 ? s.pe.toFixed(1) : "N/A", good: s.pe > 0 && s.pe <= 25, note: "Evaluare faÈ›Äƒ de profit" },
    { label: "Dividend Yield", value: `${s.divYield.toFixed(2)}%`, good: s.divYield >= 2, note: "Randament dividend" },
    { label: "Beta", value: s.beta.toFixed(2), good: s.beta <= 1.2, note: "Volatilitate faÈ›Äƒ de piaÈ›Äƒ" },
  ];

  const radarItems = [
    { label: "Profitabilitate", score: Math.min(100, s.profitMargin * 400) },
    { label: "ROE", score: Math.min(100, s.roe * 200) },
    { label: "Stabilitate (Î²)", score: Math.max(0, 100 - (s.beta - 0.5) * 66) },
  ...
  const subTabs = [
    ...
    { id: "risc", label: "Risc" },
    { id: "pozitie", label: "PoziÈ›ie" },
  ];

  return (
    <div style={{ padding: "16px 12px", display: "flex", flexDirection: "column", gap: 14 }}>
      <SectionHeader>ANALIZÄ‚ DETALIATÄ‚ PER ACTIV</SectionHeader>
      ...
      >
        {[...portfolio].sort((a, b) => a.symbol.localeCompare(b.symbol)).map(p => (
          <option key={p.symbol} value={p.symbol}>{p.symbol} â€” {p.name}</option>
        ))}
      </select>
      ...
      <Card accent={THEME.gold}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontFamily: FONT_MONO, fontSize: 11, color: THEME.gold, marginTop: 2 }}>{s.symbol} Â· {s.sector}</div>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <Card>
            <div style={{ fontSize: 9, color: THEME.dim, textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 8 }}>
              PREÈš + SMA 20 &nbsp;<span style={{ color: THEME.gold }}>â€”â€”â€”</span>&nbsp; Cost Mediu &nbsp;<span style={{ color: THEME.blue }}>â€”â€”â€”</span>&nbsp; Close
            </div>
          ...
          <Card>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
              ...
              <div style={{ display: "flex", gap: 10, fontSize: 9, color: THEME.dim }}>
                <span style={{ color: THEME.red }}>â€” 70 SupracumpÄƒrat</span>
                <span style={{ color: THEME.green }}>â€” 30 SupravÃ¢ndut</span>
              </div>
            </div>
          ...
          {/* MACD */}
          <Card>
            <div style={{ fontSize: 9, color: THEME.dim, textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 6 }}>MACD HistogramÄƒ (12/26/9)</div>
            <MACDChart closes={closes} />
            <div style={{ display: "flex", gap: 12, marginTop: 4, fontSize: 9, color: THEME.dim }}>
              <span style={{ color: THEME.green }}>â–® Pozitiv (momentum bullish)</span>
              <span style={{ color: THEME.red }}>â–® Negativ (momentum bearish)</span>
            </div>
          </Card>

          {/* 52W Range */}
          <Card>
            <div style={{ fontSize: 9, color: THEME.dim, textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 8 }}>COMPARAÈšIE 52W HIGH / LOW</div>
            ...
            <div style={{ textAlign: "center", marginTop: 6, fontSize: 10, color: THEME.dim }}>
              PoziÈ›ie Ã®n range: <span style={{ color: THEME.gold, fontFamily: FONT_MONO }}>{Math.max(0, Math.min(100, posInRange)).toFixed(1)}%</span>
            </div>
          </Card>
          ...
          {returns.length > 50 && (
            <Card>
              ...
              <div style={{ display: "flex", gap: 12, marginTop: 6, fontSize: 8, color: THEME.dim }}>
                <span>â–  <span style={{ color: THEME.green }}>Pozitiv</span></span>
                <span>â–  <span style={{ color: THEME.red }}>Negativ</span></span>
      ...
      {deepSubTab === "financiar" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {/* EPS ProiecÈ›ie */}
          <section>
            <SectionHeader>PROIECÈšIE EPS</SectionHeader>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              ...
              <Card accent={THEME.gold}>
                ...
                <div style={{ marginTop: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                    ...
                    <span style={{ fontSize: 9, color: s.payout > 80 ? THEME.red : s.payout > 65 ? THEME.gold : THEME.green }}>{s.payout > 80 ? "RISC" : s.payout > 65 ? "ATENÈšIE" : "SIGUR"}</span>
                  </div>
          ...
          {/* â”€â”€ ISTORICUL PREÈšULUI 365 ZILE â”€â”€ */}
          <section>
            <SectionHeader>ISTORICUL PREÈšULUI (365 ZILE)</SectionHeader>
            <Card>
              <PriceHistoryChart closes={closes} avgCost={s.avgCost} />
            </Card>
          </section>

          {/* â”€â”€ CANAL FCF vs DIVIDEND/SHARE â”€â”€ */}
          {s.divYield > 0 && (
            <section>
              ...
              <Card>
                ...
                <div style={{ marginTop: 10, background: THEME.bg, borderRadius: 6, padding: "8px 10px", borderLeft: `3px solid ${s.payout < 60 ? THEME.green : s.payout < 80 ? THEME.gold : THEME.red}` }}>
                  ...
                  <div style={{ fontSize: 10, color: THEME.text }}>
                    {s.payout < 60
                      ? `âœ“ FCF/Share acoperÄƒ confortabil dividendul (payout ${s.payout.toFixed(0)}%). SpaÈ›iu de creÈ™tere a dividendului.`
                      : s.payout < 80
                      ? `âš  Payout de ${s.payout.toFixed(0)}% lasÄƒ marjÄƒ moderatÄƒ. MonitorizeazÄƒ evoluÈ›ia FCF.`
                      : `âš  Payout de ${s.payout.toFixed(0)}% â€” dividendul consumÄƒ cea mai mare parte din FCF. Risc de tÄƒiere dacÄƒ FCF scade.`}
                  </div>
                </div>
              </Card>
            </section>
          ...
          {/* â”€â”€ EVOLUÈšIE VENITURI â”€â”€ */}
          <section>
            <SectionHeader>EVOLUÈšIE VENITURI</SectionHeader>
            <Card>
          ...
          <section>
            ...
            <Card>
              ...
              <div style={{ marginTop: 10, background: THEME.bg, borderRadius: 6, padding: "8px 10px", borderLeft: `3px solid ${THEME.blue}` }}>
                <div style={{ fontSize: 9, color: THEME.dim, marginBottom: 3 }}>Randament Total AcÈ›ionar (estimat)</div>
                <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
                  {[
                    ...
                    { l: "Total Shareholder", v: s.divYield > 0 ? `~${(s.divYield * 1.35).toFixed(2)}%` : "â€”", c: THEME.blue },
                  ].map(x => (
                    <div key={x.l}>
                      <div style={{ fontSize: 8, color: THEME.dim }}>{x.l}</div>
      ...
      {/* â”€â”€ SUB-TAB: RISC â”€â”€ */}
      {deepSubTab === "risc" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>

          {/* Radar performanÈ›Äƒ */}
          <section>
            <SectionHeader>RADAR PERFORMANÈšÄ‚ MULTIDIMENSIONAL</SectionHeader>
            <Card>
              {radarItems.map(item => (
          ...
          {/* VaR & Risk Metrics */}
          <section>
            <SectionHeader>DISTRIBUÈšIE RANDAMENTE & RISK METRICS</SectionHeader>
            <Card>
              {returns.length >= 20 ? (
          ...
          {/* Factori de risc */}
          <section>
            <SectionHeader>FACTORI DE RISC IDENTIFICAÈšI</SectionHeader>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                { cond: s.pe > 35 && s.pe < 500, label: "Evaluare ExcesivÄƒ", desc: `P/E de ${s.pe.toFixed(1)} depÄƒÈ™eÈ™te 35x`, color: THEME.gold },
                { cond: s.payout > 80 && s.divYield > 0 && s.sector !== "Real Estate", label: "Risc TÄƒiere Dividend", desc: `Payout de ${s.payout.toFixed(0)}% poate fi nesustenabil`, color: THEME.red },
                { cond: s.beta > 1.5, label: "Volatilitate RidicatÄƒ", desc: `Beta de ${s.beta.toFixed(2)} indicÄƒ volatilitate mare faÈ›Äƒ de piaÈ›Äƒ`, color: THEME.blue },
                { cond: s.currentRatio < 1 && s.sector !== "Financials", label: "Lichiditate ScÄƒzutÄƒ", desc: `Current Ratio ${s.currentRatio.toFixed(2)} sub 1.0`, color: THEME.red },
                { cond: s.profitMargin < 0, label: "MarjÄƒ NegativÄƒ", desc: `Compania opereazÄƒ Ã®n pierdere`, color: THEME.red },
                { cond: (s.value / totals.value) * 100 > 15, label: "Concentrare ExcesivÄƒ", desc: `Pondere de ${((s.value/totals.value)*100).toFixed(1)}% Ã®n portofoliu`, color: THEME.gold },
              ...
              {[
                ...
                <div style={{ background: THEME.bg, borderLeft: `3px solid ${THEME.green}`, borderRadius: "0 6px 6px 0", padding: "10px 12px" }}>
                  <div style={{ fontSize: 11, color: THEME.green }}>âœ“ Niciun risc major identificat</div>
                  <div style={{ fontSize: 10, color: THEME.dim, marginTop: 2 }}>ToÈ›i indicatorii sunt Ã®n parametri normali</div>
                </div>
              )}
            </div>
          </section>
      ...
      {/* â”€â”€ SUB-TAB: POZIÈšIE â”€â”€ */}
      {deepSubTab === "pozitie" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>

          {/* Detalii poziÈ›ie */}
          <section>
            <SectionHeader>DETALII POZIÈšIE</SectionHeader>
            <Card accent={THEME.gold}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                {[
                  { l: "Valoare ActualÄƒ", v: fmtUSD(s.value), c: THEME.text },
                  { l: "Valoare InvestitÄƒ", v: fmtUSD(s.invested), c: THEME.dim },
                  ...
                  { l: "Nr. AcÈ›iuni", v: s.shares, c: THEME.text },
                  { l: "PreÈ› Mediu", v: `$${fmt(s.avgCost)}`, c: THEME.text },
                  { l: "PreÈ› Curent", v: `$${fmt(s.price)}`, c: clr(s.dailyChg) },
          ...
          {/* Breakeven analysis */}
          <section>
            <SectionHeader>ANALIZÄ‚ BREAKEVEN</SectionHeader>
            <Card>
              {(() => {
                ...
                return (
                  <>
                    <MetricRow label="DiferenÈ›Äƒ faÈ›Äƒ de cost" value={`${sign(gap)}$${Math.abs(gap).toFixed(2)}`} color={clr(gap)} />
                    ...
                    <div style={{ marginTop: 10 }}>
                      <div style={{ fontSize: 9, color: THEME.dim, marginBottom: 4 }}>Recuperare investiÈ›ie prin dividende</div>
                      ...
                      <div style={{ fontSize: 9, color: THEME.dim, marginTop: 3 }}>
                        {s.annualDiv > 0
                          ? `Recuperare completÄƒ Ã®n ~${(s.invested / s.annualDiv).toFixed(0)} ani la dividendul actual`
                          : "FÄƒrÄƒ dividend â€” recuperare exclusiv prin aprecierea preÈ›ului"}
                      </div>
                    </div>
                  </>
                );
          ...
          {/* Scenarii DCA */}
          <section>
            <SectionHeader>SCENARII ACHIZIÈšIE ADIÈšIONALÄ‚</SectionHeader>
            <Card>
              <div style={{ fontSize: 10, color: THEME.dim, marginBottom: 10 }}>Impact DCA la preÈ›ul curent de ${fmt(s.price)}</div>
                ...
                return (
                  <div key={cap} style={{ padding: "8px 0", borderBottom: `1px solid ${THEME.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      ...
                      <div style={{ fontSize: 9, color: THEME.dim }}>+{newShares} acÈ› Â· nou avg ${fmt(newAvg)}</div>
                    </div>
                    <div style={{ textAlign: "right" }}>
                      <div style={{ fontFamily: FONT_MONO, fontSize: 11, color: clr(newProfit) }}>{sign(newProfit)}${Math.abs(newProfit).toFixed(0)}</div>
                      <div style={{ fontSize: 9, color: THEME.dim }}>{totalShares} acÈ› total</div>
                    </div>
                  </div>
                );
              })}
          ...
          {/* EvoluÈ›ie zilnicÄƒ vs portofoliu */}
          <section>
            <SectionHeader>CONTRIBUÈšIE LA MIÈ˜CAREA ZILNICÄ‚</SectionHeader>
            <Card>
              {(() => {
                ...
                return (
                  <>
                    <MetricRow label="MiÈ™care activ ($)" value={`${sign(stockDailyUSD)}$${Math.abs(stockDailyUSD).toFixed(2)}`} color={clr(stockDailyUSD)} />
                    <MetricRow label="VariaÈ›ie zilnicÄƒ (%)" value={`${sign(s.dailyChg)}${fmt(s.dailyChg, 2)}%`} color={clr(s.dailyChg)} />
                    <MetricRow label="ContribuÈ›ie la port." value={`${sign(contribution)}${Math.abs(contribution).toFixed(1)}%`} color={clr(contribution)} />
                    <div style={{ marginTop: 10 }}>
                      <div style={{ fontSize: 9, color: THEME.dim, marginBottom: 4 }}>ContribuÈ›ie la miÈ™carea totalÄƒ a zilei</div>
...
// â”€â”€ TAB 7: ALERTE PREÈš â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function AlerteTab({ portfolio, alerts, setAlerts }) {
  ...
  return (
    <div style={{ padding: "16px 12px", display: "flex", flexDirection: "column", gap: 16 }}>
      <SectionHeader>SISTEM ALERTE PREÈš</SectionHeader>

      {triggered.length > 0 && (
        <Card accent={THEME.gold}>
          <div style={{ fontSize: 10, color: THEME.gold, marginBottom: 8 }}>ðŸ”” ALERTE ACTIVE</div>
          {triggered.map(s => {
            const a = alerts[s.symbol];
      ...
      <div style={{ fontSize: 10, color: THEME.dim, marginBottom: -8 }}>LasÄƒ 0 pentru a dezactiva alerta</div>

      {portfolio.map(s => {
        const a = alerts[s.symbol] || { buy: 0, sell: 0 };
        const status = a.buy > 0 && s.price <= a.buy ? "â–² DECLANÈ˜AT BUY" :
                       a.sell > 0 && s.price >= a.sell ? "â–¼ DECLANÈ˜AT SELL" :
                       (a.buy > 0 || a.sell > 0) ? "â—‰ Armat" : "â€” Inactiv";
        ...
        return (
          <Card key={s.symbol}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              <div>
                <div style={{ fontSize: 9, color: THEME.green, marginBottom: 3 }}>CUMPÄ‚RÄ‚ SUB $</div>
                <input
                  type="number"
                  value={a.buy || ""}
      ...
      <div style={{
        ...
        {status === "live" && liveCount < symbols.length && (
          <div style={{ background: `${THEME.gold}18`, borderBottom: `1px solid ${THEME.gold}44`, padding: "6px 14px", fontSize: 10, color: THEME.gold, fontFamily: FONT_MONO }}>
            âš¡ {liveCount}/{symbols.length} tickere actualizate live Â· restul din cache
          </div>
        )}
        {status === "error" && (
          <div style={{ background: `${THEME.red}18`, borderBottom: `1px solid ${THEME.red}44`, padding: "6px 14px", fontSize: 10, color: THEME.red, fontFamily: FONT_MONO }}>
            âš  Date live indisponibile (CORS / ratÄƒ depÄƒÈ™itÄƒ) Â· se afiÈ™eazÄƒ date demo
          </div>
        )}
